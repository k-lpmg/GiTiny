...
final class SearchRepositoriesTableViewController: BaseViewController {
    ...
    private let sort: BehaviorRelay<GitHubSearchService.RepositoriesSort>
    private let query: Observable<(String, GitHubSearchService.RepositoriesSort)>
    private let pageRelay: BehaviorRelay<Int>
    
    private var sortActions: [RxAlertAction<String>] {
        var actions = [RxAlertAction<String>]()
        GitHubSearchService.RepositoriesSort.allCases.forEach { (sort) in
    ...
    private let tableView: UITableView = {
        ...
        tableView.register(cellType: SearchRepositoryTableViewCell.self)
        return tableView
    }()
    ...
    init(viewModel: SearchRepositoriesViewModel, searchedText: Driver<String?>) {
            ...
            .bind { [weak self] (_) in
                ...
                sinceAlertController
                    .subscribe(onNext: { [weak self] (sort) in
                        guard let self = self, let sort = GitHubSearchService.RepositoriesSort.init(rawValue: sort) else {return}
    ...
    private func bindViewModel(searchedText: Driver<String?>) {
        ...
        let request = Observable<SearchRepositoriesViewModel.Request>.create { [weak self] (observer) -> Disposable in
        ...
        let modelSelected = tableView.rx.modelSelected(SearchRepository.self).asDriver()
        ...
        output.searchRepositories
            .drive(tableView.rx.items(cellIdentifier: SearchRepositoryTableViewCell.reuseIdentifier,
                                      cellType: SearchRepositoryTableViewCell.self)) { index, model, cell in
...
extension SearchRepositoriesTableViewController {
...
extension SearchRepositoriesTableViewController: UITableViewDelegate {
